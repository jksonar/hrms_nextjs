...
def create_attendance(attendance: AttendanceCreate, db: Session = Depends(get_db), current_user: DBUser = Depends(get_current_active_user)):
...
def read_attendance(skip: int = 0, limit: int = 100, db: Session = Depends(get_db), current_user: DBUser = Depends(get_current_active_user)):
...
def read_employee_attendance(employee_id: int, start_date: date = None, end_date: date = None, db: Session = Depends(get_db), current_user: DBUser = Depends(get_current_active_user)):
...
def read_attendance_record(attendance_id: int, db: Session = Depends(get_db), current_user: DBUser = Depends(get_current_active_user)):
...
def update_attendance(attendance_id: int, attendance: AttendanceUpdate, db: Session = Depends(get_db), current_user: DBUser = Depends(get_current_active_user)):
    db_attendance = crud.get_attendance(db, attendance_id=attendance_id)
    if db_attendance is None:
        raise HTTPException(status_code=404, detail="Attendance record not found")
...
def clock_in(employee_id: int, db: Session = Depends(get_db), current_user: DBUser = Depends(get_current_active_user)):
    today = date.today()
...
def clock_out(employee_id: int, db: Session = Depends(get_db), current_user: DBUser = Depends(get_current_active_user)):
    today = date.today()
    existing_attendance = crud.get_attendance_by_employee_and_date(db, employee_id=employee_id, date=today)
...
@router.get("/attendance/my-records", response_model=List[Attendance])
def read_my_attendance(start_date: date = None, end_date: date = None, db: Session = Depends(get_db), current_user: DBUser = Depends(get_current_active_user)):
