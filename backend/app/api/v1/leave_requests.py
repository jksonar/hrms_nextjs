...
from app.db.models import User as DBUser, LeaveStatus
...
def read_leave_requests(skip: int = 0, limit: int = 100, db: Session = Depends(get_db), current_user: DBUser = Depends(get_current_active_user)):
    leave_requests = crud.get_leave_requests(db, skip=skip, limit=limit)
    return leave_requests

@router.get("/leave-requests/employee/{employee_id}", response_model=List[LeaveRequest])
def read_employee_leave_requests(employee_id: int, db: Session = Depends(get_db), current_user: DBUser = Depends(get_current_active_user)):
...
def read_leave_request(leave_request_id: int, db: Session = Depends(get_db), current_user: DBUser = Depends(get_current_active_user)):
...
def update_leave_request(leave_request_id: int, leave_request: LeaveRequestUpdate, db: Session = Depends(get_db), current_user: DBUser = Depends(get_current_active_user)):
    db_leave_request = crud.get_leave_request(db, leave_request_id=leave_request_id)
    if db_leave_request is None:
        raise HTTPException(status_code=404, detail="Leave request not found")
...
@router.get("/leave-requests/my-requests", response_model=List[LeaveRequest])
...
@router.get("/leave-requests/pending-approvals", response_model=List[LeaveRequest])
